    ...
    for (float x = -size; x < size; x += step) {
        for (float z = -size; z < size; z += step) { 
            ...
            auto calcColor = [&](float vx, float vz) { 
                ...
                if (weather == "Snowly") { 
                    if (inCrater) { r = 200.0f / 255; g = 180.0f / 255; b = 150.0f / 255; } 
                    else { r = 1.0f; g = 1.0f; b = 1.0f; } 
                } else { 
                    if (inCrater) { r = 153.0f / 255; g = 76.0f / 255; b = 0.0f;} 
                    else { r = 0.0f; g = 102.0f / 255; b = 0.0f; } 
                } 
                glColor3f(r, g, b); 
                }; 
            auto n1 = getNormal(x1, z1); 

            glNormal3f(n1.x, n1.y, n1.z); 
            calcColor(x1, z1); 
            glVertex3f(x1, y1, z1); 

            auto n2 = getNormal(x2, z1); 
            glNormal3f(n2.x, n2.y, n2.z); 
            calcColor(x2, z1); 
            glVertex3f(x2, y2, z1); 

            auto n3 = getNormal(x2, z2); 
            glNormal3f(n3.x, n3.y, n3.z); 
            calcColor(x2, z2); 
            glVertex3f(x2, y3, z2); 

            auto n4 = getNormal(x1, z2); 
